...
class Model(object):
    ...
    def input_def(self):
        self.batch_h = tf.placeholder(tf.int32, [self.batch_size * (self.neg_size + 1)])
        self.batch_t = tf.placeholder(tf.int32, [self.batch_size * (self.neg_size + 1)])
        self.batch_r = tf.placeholder(tf.int32, [self.batch_size * (self.neg_size + 1)])
        self.batch_y = tf.placeholder(tf.float32, [self.batch_size * (self.neg_size + 1)])
...
class DistMult(Model):
    ...
    def loss_def(self):
        ...
        loss_func = - tf.reduce_mean(tf.log(tf.nn.sigmoid(y * res)))
